...
func CreateIdentityMatrix4x4() *Matrix4x4 {
    ...
    mat.m[0][1] = mat.m[0][2] = mat.m[0][3] = mat.m[1][0] = 0.0
    mat.m[1][2] = mat.m[1][3] = mat.m[2][0] = mat.m[2][1] = mat.m[2][3] = 0.0
...
func HasScaleTransform(t *Transform) bool {
    la2 := LengthSquaredVector(VectorTransform(t, &Vector{ 1, 0, 0 }))
    lb2 := LengthSquaredVector(VectorTransform(t, &Vector{ 0, 1, 0 }))
    lc2 := LengthSquaredVector(VectorTransform(t, &Vector{ 0, 0, 1 }))
    return (la2 < 0.999 || la2 > 1.001 || lb2 < 0.999 || lb2 > 1.001 || lc2 < 0.999 || lc2 > 1.001)
}

func PointTransform(t *Transform, p *Point) *Point {
...
func MultTransform(t1, t2 *Transform) *Transform {
...
func RotateTransform(angle float64, axis *Vector) *Transform {
    ...
    mat := CreateMatrix4x4(a.x * a.x + (1.f - a.x * a.x) * c, a.x * a.y * (1.f - c) - a.z * s;a.x * a.z * (1.f - c) + a.y * s, 0,
                           a.x * a.y * (1.f - c) + a.z * s, a.y * a.y + (1.f - a.y * a.y) * c, a.y * a.z * (1.f - c) - a.x * s, 0,
                           a.x * a.z * (1.f - c) - a.y * s, a.y * a.z * (1.f - c) + a.x * s, a.z * a.z + (1.f - a.z * a.z) * c, 0,
                           0, 0, 0, 1)
...
func LookAtTransform(pos, look *Point, up *Vector) (*Transform, error) {
    ...
    dir := NormalizeVector(SubVector(look, pos))
    if LengthVector(CrossVector(NormalizeVector(up), dir)) == 0 {
        return nil, fmt.Errorf("\"up\" vector (%f, %f, %f) and viewing direction (%f, %f, %f) "
    ...
    return &Transform{ InverseMatrix4x4(camToWorld), camToWorld }
}

func OrthographicTransform(znear, zfar float64) *Transform {
    return MultTransform(ScaleTransform(1.0, 1.0, 1.0 / (zfar-znear)),
           TranslateTransform(&Vector{ 0.0, 0.0, -znear }))
}
...
func PerspectiveTransform(fov, znear, zfar float64) *Transform {
    ...
    return MultTransform(ScaleTransform(invTanAng, invTanAng, 1), CreateTransform(persp))
}
...
func CreateAnimatedTransform(trans1 *Transform, time1 float64, trans2 *Transform, time2 float64) *AnimatedTransform {
    ...
    t.actuallyAnimated = NotEqualTransform(tran1, trans2)
    t.t[0], t.r[0], t.s[0] = Decompose(t.startTransform.m)
    t.t[1], t.r[1], t.s[1] = Decompose(t.endTransform.m)
...
func DecomposeMatrix4x4(m *Matrix4x4) (*Vector, *Quaternion, *Matrix4x4) {
    ...
    for ++count < 100 && norm > .0001 {
        ...
        Rnext := new(Matrix4x4)
        Rit := InverseMatrix4x4(TransposeMatrix4x4(R))
        for i := 0; i < 4; ++i {
            for j := 0; j < 4; ++j {
    ...
    Rquat = CreateQuaternionMatrix4x4(R)

    // Compute scale _S_ using rotation and original matrix
    S = MulMatrix4x4(InverseMatrix4x4(R), M)

    return T, Rquat, S
}

func InterpolateAnimatedTransform(t *AnimatedTransform, time float64) *Transform {
    ...
    trans := AddVector(ScaleVector((1.0 - dt), t.t[0]), ScaleVector(dt, t.t[1]))

    // Interpolate rotation at _dt_
    rotate := SlerpQuaternion(dt, t.r[0], t.r[1])
    ...
    return MultTransform(TranslateTransform(trans), MultTransform(QuaternionToTransform(rotate), CreateTransform(scale)))
}
...
func MotionBoundsAnimatedTransform(t *AnimatedTransform, b *BBox, useInverse bool) *BBox {
    if !t.actuallyAnimated {
        return useInverse ? Inverse(*t.startTransform)(b) : (*t.startTransform)(b)
    }
    BBox ret
    nSteps := 128
    for i := 0; i < nSteps; ++i {
        ...
        Interpolate(time, trans)
        if useInverse { trans = Inverse(trans) }
        ret = Union(ret, trans(b))
    }
    return ret 
}

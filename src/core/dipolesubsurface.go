...
func (node *SubsurfaceOctreeNode) Insert(nodeBound *BBox, ip *IrradiancePoint, arena *MemoryArena) {
	...
	if node.isLeaf {
		...
		for _, ip := range localIps {
			...
			child := 0
			if ip.p.x > pMid.x { child += 4 }
			if ip.p.y > pMid.y { child += 2 }
			if ip.p.z > pMid.z { child += 1 }
...
func (node *SubsurfaceOctreeNode) InitHierarchy() {
	if node.isLeaf {
		...
		if sumWt > 0.0 { node.p = *node.p.InvScale(sumWt) }
		...
		for i := 0; i < len(node.children); i++ {
			if node.children[i] == nil { continue }
			nChildren++
...
func (node *SubsurfaceOctreeNode) Mo(nodeBound *BBox, pt *Point, Rd *DiffusionReflectance, maxError float64) Spectrum {
    ...
    if dw < node.maxError && !nodeBound.Inside(pt) {
        //PBRT_SUBSURFACE_ADDED_INTERIOR_CONTRIBUTION(const_cast<SubsurfaceOctreeNode *>(this))
        return node.Rd(DistanceSquaredPoint(pt, node.p)) * node.E * node.sumArea
    }
    ...
    if node.isLeaf {
        ...
        for i := 0; i < len(node.ips); i++ {
            ...
            mo += node.Rd(DistanceSquared(pt, node.ips[i].p)) * node.ips[i].E * node.ips[i].area
        }
    } else {
        // Recursively visit children nodes to compute $M_\roman{o}$
        pMid := nodeBound.pMin.AddPoint(&nodeBound.pMax).Scale(0.5)
        for child := 0; child < len(node.children); child++ {
            if node.children[child] == nil { continue }
            childBound := octreeChildBound(child, nodeBound, pMid)
...
func (integrator *DipoleSubsurfaceIntegrator) Preprocess(scene *Scene, camera Camera, renderer Renderer)    {
    ...
    if len(integrator.filename) != 0 {
        ...
        var fpts []float64
        var ok bool
        if ok, fpts = ReadFloatFile(filename) {
            if (len(fpts) % 8) != 0 {
                Error("Excess values (%d) in points file \"%s\"", len(fpts) % 8, filename)
            }          
    ...
    if len(pts) == 0 {
        pCamera := camera.CameraToWorld(camera.ShutterOpen(), Point(0, 0, 0))
    ...
    //ProgressReporter progress(len(pts), "Computing Irradiances")
    for i := 0; i < len(pts); i++ {
...
func (integrator *DipoleSubsurfaceIntegrator) RequestSamples(sampler Sampler, sample *Sample, scene *Scene) {
    ...
    for i := 0; i < nLights; i++ {
        ...
        integrator.lightSampleOffsets[i] = NewLightSampleOffsets(nSamples, sample)
        integrator.bsdfSampleOffsets[i] = NewBSDFSampleOffsets(nSamples, sample)
    }	
}
...
func CreateDipoleSubsurfaceIntegrator(params *ParamSet) *DipoleSubsurfaceIntegrator {
    ...
    if options.QuickRender { maxError *= 4.0; minDist *= 4.0 }
